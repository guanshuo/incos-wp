...
RUN apt-get install -y curl wget git unzip python-software-properties python-setuptools openssh-server software-properties-common debian-archive-keyring build-depgcc
...
RUN cd tengine-2.2.0 && ./configure --with-http_concat_module && make && make install
